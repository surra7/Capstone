...
class SavedActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.savedPostsRecycle.layoutManager = layoutManager
        myAdapter = MyPostAdapter(this)
        binding.savedPostsRecycle.adapter = myAdapter
        ...
        dao.getPostList()?.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (dataSnapshot in snapshot.children) {
                    ...
                    mDbRef.child(myUid).child("bookmarkList").child(key.toString()).get().addOnSuccessListener {
                        myBookmark = it.getValue().toString()
                        if (myBookmark == key.toString()) {
                            if (postList != null) {
                                datas.add(postList)
                                datas.sortByDescending { it.cntHeart }
                            }
