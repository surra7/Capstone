...
class DetailPost : AppCompatActivity() {
    ...
    var myHeartList = arrayListOf<String>()
    var myBookmarkList = arrayListOf<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*postDB.child(key).child("heartList").addValueEventListener(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                for(dataSnapshot in snapshot.children){
                    var myHeart = dataSnapshot.getValue().toString()
                    myHeartList.add(myHeart)

                    if (myHeart == myUid){
                        heartList.add(myHeart)
                        myHeartList.add(myHeart)
                    }
        ...
        /*for (userId in heartList){
            if (myUid == userId){
                binding.btnHeart.isSelected = true
            }
        }

        for (userId in bookmarkList){
            if (myUid == userId){
                binding.btnBookmark.isSelected = true
            }
        }*/
        ...
        /*for (postkey in myHeartList){
            if (key == postkey){
                binding.btnHeart.isSelected = true
            }
        }

        Toast.makeText(this, "${myHeartList}", Toast.LENGTH_SHORT).show()

        for (postkey in myBookmarkList){
            if (key == postkey){
                binding.btnBookmark.isSelected = true
            }
        }*/
